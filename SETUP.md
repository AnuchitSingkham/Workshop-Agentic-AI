...
**ต้องตั้งก่อนถึงจะเข้าเว็บได้เลย (fail-closed เหมือนกัน — คนละตัวกับ ADMIN_TOKEN ด้านบน):**

```bash
npx wrangler secret put CLASS_PASSWORD
```

รหัสผ่านเข้าเว็บทั้งเว็บ (ทุกหน้า + `/api/chat`) แจกให้คนใน class คนละคน — ล็อกอินที่ `<WORKER_URL>/login` ครั้งเดียว
จะได้ session cookie อยู่ 12 ชั่วโมง ไม่ต้องกรอกซ้ำจนกว่าจะหมดอายุ

**ต้องตั้งก่อนถึงจะใช้ `/mcp/*` จาก MCP client ภายนอกได้ (n8n, Claude Desktop/Code ฯลฯ):**

```bash
npx wrangler secret put MCP_ACCESS_TOKEN
```

ตั้งค่านี้แล้วต้องแนบ header `Authorization: Bearer <ค่าที่ตั้ง>` มาด้วยทุกครั้งที่ต่อ `/mcp/utils`,
`/mcp/google-calendar`, `/mcp/text-to-sql` จาก client ภายนอก (ดูตัวอย่าง curl ที่
[docs/module-2.1-mcp-simple-server.md](docs/module-2.1-mcp-simple-server.md))
...
เปิด `<WORKER_URL>/` จะเจอหน้า login ก่อน (เพราะตั้ง `CLASS_PASSWORD` ไว้) — กรอกรหัสผ่านแล้วจะเข้าเว็บได้ปกติ
1 ครั้งอยู่ได้ 12 ชั่วโมง จากนั้นเปิด `/chat/` ทดสอบคุยได้เลย ถ้าตั้ง `ADMIN_TOKEN` ไว้แล้วลองเปิด `/settings-mcp/`
...
`redirect_uri` ได้ตรง ๆ (ต้องสร้าง Client ID แบบ B ในข้อ 5.3 ก่อน):
...
รายละเอียดโค้ดของ flow นี้ดูใน [`docs/module-2.2-mcp-google-calendar.md`](docs/module-2.2-mcp-google-calendar.md)
...
| เปิดเว็บแล้วเด้งไป `/login` วนไม่หยุด / เข้าไม่ได้เลย | ยังไม่ได้ตั้ง `CLASS_PASSWORD` (เว็บปิดแบบ fail-closed) รัน `npx wrangler secret put CLASS_PASSWORD` แล้ว deploy ใหม่ |
...
5. **ตั้ง secret ผ่านหน้าเว็บ** (แทนคำสั่ง `npx wrangler secret put ...` ในหัวข้อ 3 ด้านบนทั้งหมด): เปิด Worker ที่
   ...
   ตามชื่อในหัวข้อ 3 (`ADMIN_TOKEN`, `CLASS_PASSWORD`, `MCP_ACCESS_TOKEN`, `GEMINI_API_KEY`/`OPENAI_API_KEY`/
   `OPENAI_COMPAT_API_KEY`, ฯลฯ) — กด Save จะ deploy ใหม่ให้อัตโนมัติ
6. หลังจากนี้ **แก้โค้ดแล้ว push ขึ้น branch `main` ของ fork ตัวเอง** (จะแก้ผ่านหน้าเว็บ GitHub หรือดึงไปแก้ในเครื่อง
   แล้ว push กลับก็ได้) — Cloudflare จะ build/deploy ให้อัตโนมัติทุกครั้ง ไม่ต้องรัน `npm run deploy` เองอีก
